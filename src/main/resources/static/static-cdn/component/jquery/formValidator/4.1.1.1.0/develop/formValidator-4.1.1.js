...
$.formValidator = 
{
	...
	showMessage : function(returnObj)
	{
		...
		{		
			...
			//wuff modify20110804 用户体验,可以为空时，验证后显示onshow
			if($.formValidator.isEmpty(id))
			{
				$.formValidator.setTipState(elem,"onShow",$.formValidator.getStatusText(elem,setting.onShow));
			}
			else
			{
				$.formValidator.setTipState(elem,"onCorrect",$.formValidator.getStatusText(elem,setting.onCorrect));
			}
		}
		return showmsg;
	},
        ...
        switch(sType)
		{
			...
			case "textarea":
				...
				if (initConfig.wideword)
				{
					...
					{
						if ((val.charCodeAt(i) >= 0x4e00 && val.charCodeAt(i) <= 0x9fa5) || val.charCodeAt(i) == 10){ 
							len += 2;
						}else {
							len++;
						}
	...
	ajaxValid : function(returnObj)
	{
		...
		var initConfig = $.formValidator.getInitConfig(validatorGroup);
		//var parm = $.formValidator.serialize(initConfig.ajaxObjects);//wuff modify20110806
		var $obj = $(initConfig.ajaxObjects);//wuff modify2014224 新增参数parmName：控件参数名
		var parm = (setting.parmName==undefined || setting.parmName=="")?$obj.serialize():setting.parmName+"="+$obj.val();
		//添加触发的控件名、随机数、传递的参数
		...
		{	
			...
			success : function(data, textStatus, jqXHR){
				...
				if((typeof lb_ret)=="string")
				{
					...
					//alert(ls_msg) wuff modify20110829
				}
...
{
	if(typeof(onShow) == "undefined"){onShow = false;};//wuff modify20110804
	return this.each(function()
	{
		var settings = this.settings;
